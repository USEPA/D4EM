...
Public Module modModelSetup
    ...
    Public Enum HspfOutputInterval As Integer
        Hourly = 2 'If simulation is hourly and BIVL=1, then this makes hourly output
        Daily = 3
        Monthly = 4
        Yearly = 5
        Never = 6
    ...
    ''' 
    ''' </summary>
    ''' <param name="aGridPervious"></param>
    ''' <param name="aMetBaseDsns"></param>
    ''' <param name="aMetWdmIds"></param>
    ''' <param name="aUniqueModelSegmentNames"></param>
    ''' <param name="aUniqueModelSegmentIds"></param>
    ''' <param name="aOutputPath"></param>
    ''' <param name="aBaseOutputName"></param>
    ''' <param name="aSubbasinLayerName"></param>
    ''' <param name="aSubbasinFieldName"></param>
    ''' <param name="aSubbasinSlopeName"></param>
    ''' <param name="aStreamLayerName"></param>
    ''' <param name="aStreamFields"></param>
    ''' <param name="aLUType">Land use layer type (0 - USGS GIRAS Shape, 1 - NLCD grid, 2 - Other shape, 3 - Other grid)</param>
    ''' <param name="aLandUseThemeName"></param>
    ''' <param name="aLUInclude"></param>
    ''' <param name="aOutletsLayerName"></param>
    ''' <param name="aPointFieldName"></param>
    ''' <param name="aPointYear"></param>
    ''' <param name="aLandUseFieldName"></param>
    ''' <param name="aLandUseClassFile"></param>
    ''' <param name="aSubbasinSegmentName"></param>
    ''' <param name="aPSRCustom"></param>
    ''' <param name="aPSRCustomFile"></param>
    ''' <param name="aPSRCalculate"></param>
    ''' <param name="aSnowOption">0 for no snow, 1 for snow</param>
    ''' <param name="aElevationLayer"></param>
    ''' <param name="aElevationUnits">empty string if no units specified</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function SetupHSPF(ByVal aGridPervious As atcControls.atcGridSource, _
        ...
        If aLUType = 0 Then
            ...
            Dim lSuccess As Boolean = CreateLanduseRecordsGIRAS(lSubbasinsSelected, lLandUseSubbasinOverlayRecords, aSubbasinLayerName, aSubbasinFieldName, _
                                                                aSnowOption, aElevationLayer, aElevationUnits)
    ...
    Public Function CreateLanduseRecordsGIRAS(ByVal aSubbasinsSelected As atcCollection, _
                                              ...
                                              Optional ByVal aElevationLayer As D4EM.Data.Layer = Nothing, _
        ...
        If aSnowOption > 0 Then
            'TODO: GisUtil.AddCentroidXYtoShapefile(lLandUsePathName & "\overlay.shp")
        End If
        ...
        For i As Integer = 1 To lTable.NumRecords
            ...
            If aSnowOption > 0 Then
                ...
                D4EM.Geo.SpatialOperations.ProjectPoint(lX, lY, aElevationLayer.Projection, D4EM.Data.Globals.GeographicProjection)
                lRec.MeanLatitude = lY
                If aElevationLayer IsNot Nothing Then
                    lMeanElev = GisUtil.GridMeanInPolygon(aElevationLayer, lLandUsePathName & "\overlay.shp", i - 1)
    ...
    ''' 
    ''' </summary>
    ''' <param name="aSubbasinsSelected"></param>
    ''' <param name="aLandUseSubbasinOverlayRecords"></param>
    ''' <param name="aSubbasinThemeName"></param>
    ''' <param name="aSubbasinFieldName"></param>
    ''' <param name="aLandUseThemeName"></param>
    ''' <param name="aLandUseFieldName"></param>
    ''' <param name="aSnowOption">0 for no snow, 1 for snow</param>
    ''' <param name="aElevationLayer"></param>
    ''' <param name="aElevationUnits"></param>
    ''' <remarks></remarks>
    ...
    Public Sub CreateLanduseRecordsGrid(ByVal aSubbasinsSelected As atcCollection, _
        ...
        If GisUtil.LayerType(lLanduseLayerIndex) = 4 Then
            ...
            Else
                ...
                GisUtil.TabulateAreas(lLanduseLayerIndex, lSubbasinLayerIndex, lAreaLS, lMeanLatLS, lMeanElevLS, aElevationLayer)
            End If
    ...
    Public Function CreateMetWDM(ByVal aWDMFileName As String,
                                 ByVal aPREC As atcTimeseries,
                                 ByVal aATEM As atcTimeseries,
                                 ByVal aWIND As atcTimeseries,
                                 ByVal aSOLR As atcTimeseries,
                                 ByVal aPEVT As atcTimeseries,
                                 ByVal aDEWP As atcTimeseries,
                                 ByVal aCLOU As atcTimeseries
                                 ) As atcWDM.atcDataSourceWDM

        '"PREC", "ATEM", "SOLR", "WIND", "PEVT"
        Dim lNewWDM As New atcWDM.atcDataSourceWDM
        If lNewWDM.Open(aWDMFileName) Then

            If aPREC IsNot Nothing Then
                aPREC.Attributes.SetValue("ID", 1)
                aPREC.Attributes.SetValue("Constituent", "PREC")
                aPREC.Attributes.SetValue("TSTYP", "PREC")
                lNewWDM.AddDataSet(aPREC, atcDataSource.EnumExistAction.ExistReplace)
            End If

            If aATEM IsNot Nothing Then
                aATEM.Attributes.SetValue("ID", 3)
                aATEM.Attributes.SetValue("Constituent", "ATEM")
                aATEM.Attributes.SetValue("TSTYP", "ATEM")
                lNewWDM.AddDataSet(aATEM, atcDataSource.EnumExistAction.ExistReplace)
            End If

            If aWIND IsNot Nothing Then
            ...
            If aSOLR IsNot Nothing Then
                aSOLR.Attributes.SetValue("ID", 5)
                aSOLR.Attributes.SetValue("Constituent", "SOLR")
                aSOLR.Attributes.SetValue("TSTYP", "SOLR")
                lNewWDM.AddDataSet(aSOLR, atcDataSource.EnumExistAction.ExistReplace)
            End If

            If aPEVT IsNot Nothing Then
                aPEVT.Attributes.SetValue("ID", 6)
                aPEVT.Attributes.SetValue("Constituent", "PEVT")
                aPEVT.Attributes.SetValue("TSTYP", "PEVT")
                lNewWDM.AddDataSet(aPEVT, atcDataSource.EnumExistAction.ExistReplace)
            End If

            If aDEWP IsNot Nothing Then
            ...
            If aCLOU IsNot Nothing Then
                aCLOU.Attributes.SetValue("ID", 8)
                aCLOU.Attributes.SetValue("Constituent", "CLOU")
                aCLOU.Attributes.SetValue("TSTYP", "CLOU")
                lNewWDM.AddDataSet(aCLOU, atcDataSource.EnumExistAction.ExistReplace)
            End If
    ...
    Public Function CreateUCI(ByVal aUciName As String,
                              ...
                              ByVal aCalledFromFrames As Boolean,
                              ...
                              ByVal aIntermediateLocations As atcCollection,
                              ByVal aUpstreamLocations As atcCollection) As Boolean
        ...
        Dim lProjectDataSource As atcWDM.atcDataSourceWDM
        Dim lProjectWDMName As String = IO.Path.GetFileNameWithoutExtension(aUciName) & ".wdm"

        'open project wdm
        lProjectDataSource = atcDataManager.DataSourceBySpecification(IO.Path.GetFullPath(lProjectWDMName))
        If lProjectDataSource Is Nothing Then 'need to open it here
            lProjectDataSource = New atcWDM.atcDataSourceWDM
            If Not lProjectDataSource.Open(lProjectWDMName) Then
                lProjectDataSource = Nothing
            End If
        End If
        If lProjectDataSource IsNot Nothing Then
        ...
        For Each lWdmName As String In aMetWdmNames
            ...
            If aCalledFromFrames AndAlso CurDir() <> IO.Path.GetDirectoryName(lWdmName) Then
                ...
                TryCopy(lWdmName, lWdmNameForFrames)
            End If
            ...
            If lDataSource Is Nothing Then 'need to open it here
                ...
                If lDataSource.Open(lWdmNameForFrames) Then
                    'atcDataManager.DataSources.Add(lDataSource) -- pbd not sure we want this, already in project in met folder
                Else
                    lDataSource = Nothing
                End If
            End If
        ...
        If lWatershed.Open(lWatershedName) = 0 Then  'everything read okay, continue
            ...
            lHspfUci.CreateUciFromBASINS(lWatershed, _
                                         ...
                                         lStarterUciName, _
                                         aWQConstituents, _
                                         lPollutantListFileName, , , aSnowOption, True, _
                                         Jday(aSimulationStartYear, 1, 1, 0, 0, 0), _
                                         Jday(aSimulationEndYear, 12, 30, 24, 0, 0))   'for some reason this comes out as 12/31 in the uci
            'If aSimulationStartYear > 0 AndAlso aSimulationStartYear > lHspfUci.GlobalBlock.SDate(0) Then
            '    lHspfUci.GlobalBlock.SDate(0) = aSimulationStartYear
            'End If
            'If aSimulationEndYear > 0 AndAlso aSimulationEndYear < lHspfUci.GlobalBlock.EDate(0) Then
            '    lHspfUci.GlobalBlock.EDate(0) = aSimulationEndYear
            'End If

            'output required, turn on binary output at monthly interval (used to be daily or less, but size became a problem)
            Dim lOperationTypes() As String = {"PERLND", "IMPLND", "RCHRES"}
            For Each lOperationType As String In lOperationTypes
                Dim lOperations As HspfOperations = lHspfUci.OpnBlks(lOperationType).Ids
                For lOperationIndex As Integer = 1 To lOperations.Count
                    Dim lHspfOperation As HspfOperation = lOperations(lOperationIndex - 1)
                    Dim lHspfTable As HspfTable = lHspfOperation.Tables("BINARY-INFO")
                    With lHspfTable
                        For lIndex As Integer = 0 To .Parms.Count - 3
                            .Parms(lIndex).Value = 4  'aOutputInterval
                        Next
                        .Parms(.Parms.Count - 2).Value = 1 'Set BIVL=1
                    End With
                Next
            Next
            ...
            If aBacterialOption Then
                ...
                If Not lProjectDataSource.Open(lProjectWDMName) Then
                    lProjectDataSource = Nothing
                End If
                AddMicrobialOutputToWDM(lHspfUci, lProjectDataSource, aOutputInterval + 1)
                lHspfUci.Save()  'should not have to save and read again, but this is needed to handle monthdata tables
                lHspfUci.FastReadUciForStarter(lMsg, aUciName)
                RunMicrobialSourceModule(lHspfUci, aUciName)
            End If

            If aChemicalOption Then
                AddChemicalSimulation(lHspfUci, aChemicalName, aChemicalMaximumSolubility, _
    ...
    Public Sub FindUniqueMetSegments(ByRef aUniqueModelSegmentNames As atcCollection, _
    ...
    Public Sub SetPerviousGrid(ByVal aGridPervious As atcControls.atcGridSource, _
        ...
        If aGridPervious Is Nothing Then Exit Sub
                ...
                If aLayerNameLandUse.Length > 0 Then
                    ...
                    If GisUtil.LayerType(lLayerIndex) = 4 Then 'Grid
                        ...
                        For lGridX As Integer = Convert.ToInt32(GisUtil.GridLayerMinimum(lLayerIndex)) To Convert.ToInt32(GisUtil.GridLayerMaximum(lLayerIndex))
                            .Rows += 1
                            .CellValue(.Rows - 1, 0) = lGridX
                            .CellValue(.Rows - 1, 1) = lGridX
    ...
    Private Sub FillListUniqueLandUses(ByVal aGridPervious As atcControls.atcGridSource, ByVal aLayerIndex As Long, ByVal aFieldName As String)
    ...
    Public Function ReclassifyLandUses(ByVal aReclassifyFile As String, _
                                       ByVal aGridPervious As atcControls.atcGridSource, _
        ...
        For lRowIndex As Integer = 1 To aGridPervious.Rows
            lUniqueLugroups.Add(aGridPervious.CellValue(lRowIndex, 1))
        Next lRowIndex
        Logger.Dbg("GridRowCount:" & aGridPervious.Rows & " UniqueLugroupCount:" & lUniqueLugroups.Count)
        ...
        If lUseSimpleGrid Then
            For Each lLandUse As LandUse In aLandUses
                ...
                If lLandUseName IsNot Nothing Then
                    ...
                    For lRowIndex As Integer = 1 To aGridPervious.Rows
                        If lLandUseName = aGridPervious.CellValue(lRowIndex, 1) Then
                            If Double.TryParse(aGridPervious.CellValue(lRowIndex, 2), lPercentImperv) Then
                                Exit For
                            Else
                                Logger.Dbg("Warning: non-parsable percent impervious value at row " & lRowIndex & " '" & aGridPervious.CellValue(lRowIndex, 2) & "' for land use name " & lLandUseName)
                            End If
                        End If
                    Next lRowIndex
        ...
        Else 'using custom table for landuse classification
            For Each lLandUse As LandUse In aLandUses
                ...
                For lSourceRowIndex As Integer = 1 To aGridPervious.Rows
                    Dim lLandUseName As String = ""
                    Dim lpos As Integer = -1
                    Dim lPercentImperv As Double
                    If aGridPervious.CellValue(lSourceRowIndex, 0) <> "" Then
                        If lLandUse.Code = aGridPervious.CellValue(lSourceRowIndex, 0) Then
                            'see if any of these are subbasin-specific
                            If Not Double.TryParse(aGridPervious.CellValue(lSourceRowIndex, 2), lPercentImperv) Then
                                Logger.Dbg("Warning: non-parsable percent impervious value at row " & lSourceRowIndex & " '" & aGridPervious.CellValue(lSourceRowIndex, 2) & "' for land use code " & lLandUse.Code)
                            Else
                                Dim lMultiplier As Double
                                If Not Double.TryParse(aGridPervious.CellValue(lSourceRowIndex, 3), lMultiplier) Then
                                    lMultiplier = 1.0
                                End If
                                Dim lSubbasin As String = aGridPervious.CellValue(lSourceRowIndex, 4)
                                ...
                                If lSubbasinSpecific Then
                                    ...
                                    If lSubbasin = lLandUse.ModelID Then
                                        lLandUseName = aGridPervious.CellValue(lSourceRowIndex, 1)
                                    End If
                                Else
                                    ...
                                    For k As Integer = 1 To aGridPervious.Rows
                                        If k <> lSourceRowIndex Then
                                            If aGridPervious.CellValue(k, 0) = aGridPervious.CellValue(lSourceRowIndex, 0) Then
                                                'this other row has same lucode
                                                If aGridPervious.CellValue(k, 1) = aGridPervious.CellValue(lSourceRowIndex, 1) Then
                                                    'and the same group name
                                                    lSubbasin = aGridPervious.CellValue(k, 4)
...
Public Class LandUseSubbasinOverlayRecord
