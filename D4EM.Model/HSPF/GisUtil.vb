...
Public Class GisUtil
    Private Shared pMapWin As D4EM.Data.Project 'MapWindow.Interfaces.IMapWin
    Private Const UseCurrent As Integer = -1
    Private Shared pStatusShow As Boolean = True
    Private Shared pCurrentLayer As Integer = 0

    ''' <summary>Map Window Object</summary>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    <CLSCompliant(False)> _
    Public Shared WriteOnly Property MappingObject() As D4EM.Data.Project
        Set(ByVal aNewValue As D4EM.Data.Project) 'MapWindow.Interfaces.IMapWin)
            pMapWin = aNewValue
        End Set
    End Property
    ...
    ''' <summary>Map Window Object</summary>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property GetMappingObject() As D4EM.Data.Project 'MapWindow.Interfaces.IMapWin TODO: make MW6 version available from mapwindow6\\MapWindow\MwDesktop\Legacy\IMapWin.cs
        Get
    ...
    Public Shared Function Layers() As System.Collections.ObjectModel.ObservableCollection(Of D4EM.Data.Layer)
        Return GetMappingObject.Layers
    End Function
    ...
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared Function FeatureIndexValid(ByVal aFeatureIndex As Integer, ByVal aSf As D4EM.Data.Layer) As Boolean
        If aFeatureIndex < 0 Or aFeatureIndex >= aSf.AsFeatureSet.Features.Count Then
            Throw New Exception("GisUtil:FieldValue:Error:FeatureIndex:" & aFeatureIndex & ":OutOfRange:0:" & aSf.AsFeatureSet.Features.Count - 1)
        Else
            Return True
        End If
    End Function
    ...
    Public Shared Property CurrentLayer() As Integer
        Get
            ...
            'For Each llayer As D4EM.Data.Layer In GetMappingObject.Layers
    ...
    ''' <exception cref="System.Exception" caption="LayerIndexOutOfRange">Layer specified by aLayerIndex does not exist</exception>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property ShapeFileFromIndex(Optional ByVal aLayerIndex As Integer = UseCurrent) As D4EM.Data.Layer
        Get
            Return LayerFromIndex(aLayerIndex)
            If aLayerIndex = UseCurrent Then
                aLayerIndex = CurrentLayer
            End If

            Return pMapWin.Layers(aLayerIndex).DataSet
        End Get
    End Property
    ...
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property PolygonShapeFileFromIndex(ByVal aLayerIndex As Integer) As D4EM.Data.Layer
        Get
            Return ShapeFileFromIndex(aLayerIndex)
        End Get
    End Property
    ...
    ''' <exception cref="System.Exception" caption="LayerIndexOutOfRange">Layer specified by aLayerIndex does not exist</exception>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property GridFromIndex(Optional ByVal aLayerIndex As Integer = UseCurrent) As DotSpatial.Data.Raster
        Get
            If aLayerIndex = UseCurrent Then
                aLayerIndex = CurrentLayer
            End If
    ...
    ''' <exception cref="System.Exception" caption="LayerIndexOutOfRange">Layer specified by aLayerIndex does not exist</exception>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property LayerFromIndex(Optional ByVal aLayerIndex As Integer = UseCurrent) As D4EM.Data.Layer
        Get
            If aLayerIndex = UseCurrent Then
                aLayerIndex = CurrentLayer
            End If

            If aLayerIndex >= 0 AndAlso aLayerIndex < GetMappingObject.Layers.Count Then
                Return (GetMappingObject.Layers(aLayerIndex))
            Else
                Throw New Exception("GisUtil:LayerFromIndex:Error:LayerIndex:" & aLayerIndex & ":OutOfRange:0:" & GetMappingObject.Layers.Count - 1)
            End If
        End Get
    End Property
    ...
    Public Shared Function OverlappingPolygons( _
      ...
      ByVal aLayerIndex2 As Integer, ByVal aFeatureIndex2 As Integer) As Boolean
        ...
        Dim lSf1 As D4EM.Data.Layer = PolygonShapeFileFromIndex(aLayerIndex1)
        Dim lSf1Shape As DotSpatial.Data.Feature
        If FeatureIndexValid(aFeatureIndex1, lSf1) Then
            lSf1Shape = lSf1.Feature(aFeatureIndex1)

            Dim lSf2 As D4EM.Data.Layer = PolygonShapeFileFromIndex(aLayerIndex2)
            Dim lSf2Shape As DotSpatial.Data.Feature
            If FeatureIndexValid(aFeatureIndex2, lSf2) Then
                lSf2Shape = lSf2.Feature(aFeatureIndex2)

                Dim lIndex As Integer
                Dim lX As Double, lY As Double

                If Not (lSf1.AsFeatureSet.Extent.MinX > lSf2.AsFeatureSet.Extent.MaxX OrElse _
                        lSf1.AsFeatureSet.Extent.MaxX < lSf2.AsFeatureSet.Extent.MinX OrElse _
    ...
    '    Dim lLineSf As D4EM.Data.Layer = ShapeFileFromIndex(aLineLayerIndex)
    '    '5/26/09 apparent coding error corrected by LCW
    '    'Dim lLineShape As DotSpatial.Data.Feature = lLineSf.Shape(aLineIndex - 1)
    '    Dim lLineShape As DotSpatial.Data.Feature = lLineSf.Shape(aLineIndex)
    '    Dim lPolygonSf As D4EM.Data.Layer = PolygonShapeFileFromIndex(aPolygonLayerIndex)
    '    Dim lX As Double, lY As Double
    '    Dim lLineInPolygon As Integer = False
    '    For i As Integer = 1 To lLineShape.numPoints
    '        lX = lLineShape.Coordinates(i - 1).X
    '        lY = lLineShape.Coordinates(i - 1).Y
    ...
    ''' <summary>given a polygon layer (like dem shape) and a containing layer (like subbasins),
    '''  return which polygon in the containing layer each polygon lies within</summary>
    ''' <param name="aLayerIndex">Index of Layer containing polygon ShapeFile</param>
    ''' <param name="aLayerIndexContaining">Index of Layer containing polygon ShapeFile</param>
    ''' <param name="aIndex">Index of polygons from contained</param>
    ''' <exception cref="System.Exception" caption="LayerNotPolygonShapeFile">Layer specified by aLayerIndexContaining is not a polygon ShapeFile</exception>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Public Shared Sub AssignContainingPolygons(ByVal aLayerIndex As Integer, _
                                               ByVal aLayerIndexContaining As Integer, _
                                               ByRef aIndex() As Integer)
        Dim lSf1 As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
        Dim lSf2 As D4EM.Data.Layer = PolygonShapeFileFromIndex(aLayerIndexContaining)

        Dim lX As Double, lY As Double
        Dim lSf1Shape As New DotSpatial.Data.Feature
        Dim lNth As Integer

        ReDim aIndex(lSf1.AsFeatureSet.Features.Count)

        For lShapeIndex As Integer = 1 To lSf1.AsFeatureSet.Features.Count
            ...
            End If
            If aIndex(lShapeIndex) = -1 Then
                Logger.Dbg("AssignContainingPolygons: No polygon found containing stream at index " & lShapeIndex)
            End If
        Next lShapeIndex
    End Sub
    ...
    Public Shared ReadOnly Property NumFields(Optional ByVal aLayerIndex As Integer = UseCurrent) As Integer
        Get
            Return ShapeFileFromIndex(aLayerIndex).AsFeatureSet.DataTable.Columns.Count
        End Get
    End Property
    ...
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Private Shared ReadOnly Property FieldIndexValid(ByVal aFieldIndex As Integer, ByVal aSf As D4EM.Data.Layer) As Boolean
        Get
            If aFieldIndex < aSf.AsFeatureSet.DataTable.Columns.Count AndAlso aFieldIndex >= 0 Then
                Return True
            Else
                Throw New Exception("GisUtil:FieldName:Error:FieldIndex:" & aFieldIndex & ":OutOfRange:0:" & aSf.AsFeatureSet.DataTable.Columns.Count - 1)
            End If
        End Get
    End Property
    ...
    Public Shared ReadOnly Property FieldName(ByVal aFieldIndex As Integer, ByVal aLayerIndex As Integer) As String
        Get
            ...
            If FieldIndexValid(aFieldIndex, lSf) Then
                Return lSf.AsFeatureSet.DataTable.Columns(aFieldIndex).ColumnName
            Else
                Throw New Exception("GisUtil:FieldName:Error:FieldIndex:" & aFieldIndex & ":OutOfRange:0:" & lSf.AsFeatureSet.DataTable.Columns.Count - 1)
            End If
        End Get
    End Property
    ...
    Public Shared Function FieldIndex(ByVal aLayerIndex As Integer, ByVal aFieldName As String) As Integer
        Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
        For iFieldIndex As Integer = 0 To lSf.AsFeatureSet.DataTable.Columns.Count - 1
            If lSf.AsFeatureSet.DataTable.Columns(iFieldIndex).ColumnName.ToUpper = aFieldName.ToUpper Then 'this is the field we want
                Return iFieldIndex
            End If
        Next
    ...
    Public Shared Function IsField(ByVal aLayerIndex As Integer, _
        ...
        Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
        For lFieldIndex As Integer = 0 To lSf.AsFeatureSet.DataTable.Columns.Count - 1
            If lSf.AsFeatureSet.DataTable.Columns(lFieldIndex).ColumnName.ToUpper = aFieldName.ToUpper Then 'this is the field we want
                Return True
            End If
        Next
    ...
    Public Shared Function AddField(ByVal aLayerIndex As Integer, ByVal aFieldName As String, _
                                    ByVal aFieldType As Integer, ByVal aFieldWidth As Integer) As Integer
        Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
        Return AddField(aLayerIndex, aFieldName, aFieldType, aFieldWidth, lSf.AsFeatureSet.DataTable.Columns.Count)
    End Function
    ...
    Public Shared Function AddField(ByVal aLayerIndex As Integer, ByVal aFieldName As String, _
        ...
        End With
        lSf.AsFeatureSet.DataTable.Columns.Add(lNewField)

        Return lSf.AsFeatureSet.DataTable.Columns.Count - 1
    End Function
    ...
    Public Shared Function RemoveField(ByVal aLayerIndex As Integer, ByVal aFieldIndex As Integer) As Boolean
        ShapeFileFromIndex(aLayerIndex).AsFeatureSet.DataTable.Columns.RemoveAt(aFieldIndex)
        Return True
    End Function
    ...
    Public Shared ReadOnly Property NumLayers() As Integer
        Get
            Return GetMappingObject.Layers.Count()
        End Get
    End Property
    ...
    Public Shared ReadOnly Property LayerName(Optional ByVal aLayerIndex As Integer = UseCurrent) As String
        Get
            Return LayerFromIndex(aLayerIndex).Specification.Name
        End Get
    End Property
    ...
    Public Shared Function LayerIndex(ByVal aLayerName As String) As Integer
        ...
        For lLayerIndex = GetMappingObject.Layers.Count - 1 To 0 Step -1
            If LayerName(lLayerIndex).ToUpper = aLayerName OrElse LayerFileName(lLayerIndex).ToUpper = aLayerName Then
                Return lLayerIndex
            End If
        Next

        If Integer.TryParse(aLayerName, lLayerIndex) AndAlso lLayerIndex >= 0 AndAlso lLayerIndex < GetMappingObject.Layers.Count Then
            Return lLayerIndex
        End If
    ...
    Public Shared ReadOnly Property NumFeatures(Optional ByVal aLayerIndex As Integer = UseCurrent) As Integer
        Get
            Return ShapeFileFromIndex(aLayerIndex).AsFeatureSet.Features.Count
        End Get
    End Property
    ...
    '    Get
    '        Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
    '        If aFieldIndex < 0 Or aFieldIndex >= lSf.AsFeatureSet.DataTable.Columns.Count Then
    '            Throw New Exception("GisUtil:FieldType:Error:FieldIndex:" & aFieldIndex & ":OutOfRange:0:" & lSf.AsFeatureSet.DataTable.Columns.Count - 1)
    '        Else
    '            Return lSf.AsFeatureSet.DataTable.Columns(aFieldIndex).DataType
    '        End If
    '    End Get
    'End Property
    ...
    Public Shared Function FieldValue(ByVal aLayerIndex As Integer, ByVal aFeatureIndex As Integer, ByVal aFieldIndex As Integer) As String
        Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aLayerIndex)
        If aFieldIndex < 0 Or aFieldIndex >= lSf.AsFeatureSet.DataTable.Columns.Count Then
            Throw New Exception("GisUtil:FieldValue:Error:FieldIndex:" & aFieldIndex & ":OutOfRange:0:" & lSf.AsFeatureSet.DataTable.Columns.Count - 1)
    ...
    '    lPoint.x = aX
    '    lPoint.y = aY
    '    Dim lsf As D4EM.Data.Layer = ShapeFileFromIndex(aLineLayerIndex)
    '    If FeatureIndexValid(aLineFeatureIndex, lsf) Then
    '        Dim lShape As DotSpatial.Data.Feature = lsf.Shape(aLineFeatureIndex)
    ...
    ''' <exception cref="System.Exception" caption="FeatureIndexOutOfRange">Feature Index Out of Range</exception>
    ''' <exception cref="MappingObjectNotSetException">Mapping Object Not Set</exception>
    Public Shared Sub RemoveFeature(ByVal aLayerIndex As Integer, ByVal aFeatureIndex As Integer)
        RemoveFeatureNoStartStop(aLayerIndex, aFeatureIndex)
    End Sub

    Public Shared Sub StartRemoveFeature(ByVal aLayerIndex As Integer)
        'No action needed in DotSpatial
    End Sub

    Public Shared Sub RemoveFeatureNoStartStop(ByVal aLayerIndex As Integer, ByVal aFeatureIndex As Integer)
        ShapeFileFromIndex(aLayerIndex).AsFeatureSet.Features.RemoveAt(aFeatureIndex)
    End Sub
    ...
    Public Shared Sub SetFeatureValue(ByVal aLayerIndex As Integer, ByVal aFieldIndex As Integer, ByVal aFeatureIndex As Integer, ByVal aValue As Object)
        SetFeatureValueNoStartStop(aLayerIndex, aFieldIndex, aFeatureIndex, aValue)
    End Sub
    ...
    Public Shared Sub SetFeatureValueNoStartStop(ByVal aLayerIndex As Integer, ByVal aFieldIndex As Integer, ByVal aFeatureIndex As Integer, ByVal aValue As Object)
        ...
        If FeatureIndexValid(aFeatureIndex, lSf) Then
            If FieldIndexValid(aFieldIndex, lSf) Then
                lSf.CellValue(aFieldIndex, aFeatureIndex) = aValue
            End If
        End If
    End Sub
    ...
    '    Dim lsf As D4EM.Data.Layer = ShapeFileFromIndex(lInputLayerIndex)

    '    Dim osf As New D4EM.Data.Layer
    ...
    '    End If
    '    For i = 1 To lsf.AsFeatureSet.DataTable.Columns.Count
    ...
    '        lRetc = osf.EditInsertField([of], osf.AsFeatureSet.DataTable.Columns.Count)
    '        [of] = Nothing
    '    Next i
    ...
    '        For i = 1 To lsf.AsFeatureSet.Features.Count
    ...
    '                    osf.EditInsertShape(lsf.Shape(i - 1), osf.AsFeatureSet.Features.Count)
    '                    For k = 1 To osf.AsFeatureSet.DataTable.Columns.Count
    '                        lRetc = osf.EditCellValue(k - 1, osf.AsFeatureSet.Features.Count - 1, lsf.CellValue(k - 1, i - 1))
    '                    Next k
    '                End If
    '            Next j
    '        Next i
    ...
    Public Shared Sub ExtentsOfLayer(ByVal aLayerIndex As Integer, _
        ...
        With ShapeFileFromIndex(aLayerIndex).AsFeatureSet.Extent
            aXMax = .MaxX
            aXMin = .MinX
            aYMax = .MaxY
            aYMin = .MinY
    ...
    Public Shared Function ProjectProjection() As String
        Return GetMappingObject.DesiredProjection.ToProj4String
    End Function
    ...
    '    D4EM.Geo.SpatialOperations.ProjectPoint(aX, aY,
    '                                            D4EM.Data.Globals.FromProj4(aInputProjection),
    '                                            D4EM.Data.Globals.FromProj4(aOutputProjection))
    'End Sub
    ...
    Public Shared Function PointInPolygonXY(ByVal aX As Double, ByVal aY As Double, ByVal aPolygonLayerIndex As Integer) As Integer
        Dim lPolygonSf As D4EM.Data.Layer = PolygonShapeFileFromIndex(aPolygonLayerIndex)
        Return lPolygonSf.CoordinatesInShapefile(aX, aY)
    End Function
    ...
    Public Shared Function AddLayer(ByVal aFileName As String, _
        ...
        Else
            If Not FileExists(aFileName) Then
                Return False
            Else
                pMapWin.Layers.Add(New D4EM.Data.Layer(aFileName, New D4EM.Data.LayerSpecification(Tag:=aLayerName, Name:=aLayerName)))
                Return True
            End If
        End If
    End Function
    ...
    Public Shared Function IsLayer(ByVal aLayerName As String) As Boolean
        ...
        For i As Integer = 0 To GetMappingObject.Layers.Count - 1
            Try
                If lLayerName = LayerFromIndex(i).Specification.Name.ToUpper OrElse lLayerName = LayerFromIndex(i).FileName.ToUpper Then
                    Return True
                End If
            Catch
                Logger.Dbg("Layer " & i & " of " & GetMappingObject.Layers.Count & " Problem")
            End Try
        Next i
        Return False
    End Function
    ...
    Public Shared Function RemoveLayer(Optional ByVal aLayerIndex As Integer = UseCurrent) As Boolean
        If aLayerIndex = UseCurrent Then aLayerIndex = CurrentLayer
        GetMappingObject.Layers.RemoveAt(aLayerIndex)
        Return True
    End Function
    ...
    Public Shared Sub TabulateAreas(ByVal aGridLayerIndex As Integer, _
                                    ...
                                    Optional ByVal aElevationLayer As D4EM.Data.Layer = Nothing)
        ...
        'set input grid
        Dim lInputGrid As DotSpatial.Data.Raster = GridFromIndex(aGridLayerIndex)
        'set input polygon layer
        Dim lPolygonSf As D4EM.Data.Layer = PolygonShapeFileFromIndex(aPolygonLayerIndex)

        'if we want to compute mean elevation at the same time, need to open the elev grid
        Dim lElevGrid As DotSpatial.Data.Raster = Nothing
        If aElevationLayer IsNot Nothing Then
            lElevGrid = aElevationLayer.AsRaster
        End If
        ...
        lMinX = lPolygonSf.AsFeatureSet.Extent.MinX
        lMaxX = lPolygonSf.AsFeatureSet.Extent.MaxX
        lMinY = lPolygonSf.AsFeatureSet.Extent.MinY
        lMaxY = lPolygonSf.AsFeatureSet.Extent.MaxY
        ...
        '        If lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.X < lMinX Then
        '            lMinX = lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.X
        '        End If
        '        If lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.Y < lMinY Then
        '            lMinY = lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.Y
        '        End If
        '        If lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.Maximum.X > lMaxX Then
        '            lMaxX = lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.Maximum.X
        '        End If
        '        If lPolygonSf.AsFeatureSet.Features(lPolyIndex)..Envelope.Maximum.Y > lMaxY Then
        '            lMaxY = lPolygonSf.AsFeatureSet.Features(lPolyIndex)..Envelope.Maximum.Y
        '        End If
        '    Next
        'End If

        Dim lStartingRow As Integer, lStartingColumn As Integer
        Dim lEndingRow As Integer, lEndingColumn As Integer
        D4EM.Data.Layer.CellBoundsRaster(lInputGrid, lMinX, lMaxY, lMaxX, lMinY,
                                         lStartingRow, lStartingColumn,
                                         lEndingRow, lEndingColumn)

        Dim lCellArea As Double = lInputGrid.CellWidth * lInputGrid.CellHeight
        ...
        Dim lElevRowCol As DotSpatial.Data.RcIndex
        Dim lElevEndingRow As Integer = 0
        Dim lElevEndingCol As Integer = 0
        If lElevGrid IsNot Nothing Then
            lElevEndingRow = lElevGrid.NumRows
            lElevEndingCol = lElevGrid.NumColumns
        End If
        ...
        For lRow As Integer = lStartingRow To lEndingRow
            If aMeanLatGridPoly IsNot Nothing Then
                lCoordinates = DotSpatial.Data.RasterExt.CellToProj(lInputGrid, lRow, lStartingColumn)
                D4EM.Geo.SpatialOperations.ProjectPoint(lCoordinates.X, lCoordinates.Y, lInputGrid.Projection, D4EM.Data.Globals.GeographicProjection)  'used for computing mean latitude
            ...
            For lCol As Integer = lStartingColumn To lEndingColumn
                ...
                If lInsideId > -1 Then 'this is in a subbasin
                    ...
                    'changes below by mog 6/2015
                    If lSByte Then
                        lGridValue = Convert.ToInt32(lInputGrid.Value(lRow, lCol))
                        'added conditional test for our 64bit nlcd grid so we're not trying to stuff minus 10 billion into 32bit int cells for no data values
                    ElseIf lInputGrid.Value(lRow, lCol) > 0 Then
                        lGridValue = lInputGrid.Value(lRow, lCol)
                        'and added else for nodata values (instread of -1(10)**10)
                    Else
                        lGridValue = -1
                    End If
                    If lGridValue > -1 AndAlso lGridValue <= aAreaGridPoly.GetUpperBound(0) Then
                    ...
                    If aMeanElevGridPoly IsNot Nothing AndAlso lElevGrid IsNot Nothing Then
                        If lGridValue > -1 AndAlso lGridValue <= aMeanElevGridPoly.GetUpperBound(0) Then
                            ...
                            lCoordinates = DotSpatial.Data.RasterExt.CellToProj(lInputGrid, lRow, lCol)
                            lElevRowCol = DotSpatial.Data.RasterExt.ProjToCell(lElevGrid, lCoordinates.X, lCoordinates.Y)
                            If lElevRowCol.Column > -1 AndAlso lElevRowCol.Column < lElevEndingCol And _
                               lElevRowCol.Row > -1 AndAlso lElevRowCol.Row < lElevEndingRow Then
                                lElevValue = lElevGrid.Value(lElevRowCol.Row, lElevRowCol.Column)
                                If lElevValue <> lElevGrid.NoDataValue AndAlso lElevValue > -100000 Then
    ...
    '    'set input polygon layer
    '    Dim lPolygonSf As D4EM.Data.Layer = PolygonShapeFileFromIndex(aPolygonLayerIndex)
    '    Dim lShape As New DotSpatial.Data.Feature
    '    If FeatureIndexValid(aPolygonFeatureIndex, lPolygonSf) Then
    '        lShape = lPolygonSf.AsFeatureSet.Features(aPolygonFeatureIndex)

    '        'figure out what part of the grid overlays this polygon
    '        lInputGrid.ProjToCell(lShape.Envelope.X, lShape.Envelope.Y, lStartCol, lEndRow)
    '        lInputGrid.ProjToCell(lShape.Envelope.Maximum.X, lShape.Envelope.Maximum.Y, lEndCol, lStartRow)
    ...
    '                lSubId = lPolygonSf.CoordinatesInShapefile(lXPos, lYPos)
    '                If lSubId = aPolygonFeatureIndex Then 'this is in the polygon we want
    '                    lVal = lInputGrid.Value(lRow, lCol)
    '                    If lVal > aMax Then
    '                        aMax = lVal
    '                    End If
    '                    If lVal < aMin Then
    ...
    '    Dim lPolygonSf As New D4EM.Data.Layer
    ...
    'Public Shared GridMeanInPolygonFeatureIndexLayer As DotSpatial.Data.Raster(Of Integer) = Nothing

    'Private Shared Function ComputeGridMeanInPolygon(ByVal aGridLayerName As String, ByVal aPolygonLayer As D4EM.Data.Layer, ByVal aPolygonFeatureIndex As Integer) As Double
    '    'Given a grid and a polygon layer, find the mean grid value within the feature.
    '    Dim lMean As Double = -999.0
    '    If FeatureIndexValid(aPolygonFeatureIndex, aPolygonLayer) Then
    '        'set input grid
    '        Dim lInputGrid = DotSpatial.Data.Raster.Open(aGridLayerName)

    '        Dim lShape As New DotSpatial.Data.Feature
    '        lShape = aPolygonLayer.AsFeatureSet.Features(aPolygonFeatureIndex)

    '        'figure out what part of the grid overlays this polygon

    '        Dim lStartRow As Integer, lStartCol As Integer
    '        Dim lEndRow As Integer, lEndCol As Integer
    '        With aPolygonLayer.AsFeatureSet.Extent
    '            D4EM.Data.Layer.CellBoundsRaster(lInputGrid, .X, .Y, .MaxX, .MaxY,
    '                                             lStartRow, lStartCol, lEndRow, lEndCol)
    '        End With
    '        Dim lCellcount As Integer = 0

    '        If GridMeanInPolygonFeatureIndexLayer Is Nothing Then
    '            GridMeanInPolygonFeatureIndexLayer = DotSpatial.Data.Raster.Create(IO.Path.ChangeExtension(lInputGrid.Filename, ".polygons" & IO.Path.GetExtension(lInputGrid.Filename)),
    '                                                                               lInputGrid.DriverCode, lInputGrid.NumRows, lInputGrid.NumColumns, 1, GetType(Integer), New String() {""})
    '            GridMeanInPolygonFeatureIndexLayer.NoDataValue = -1
    '            GridMeanInPolygonFeatureIndexLayer.Bounds = lInputGrid.Bounds.Copy
    '            GridMeanInPolygonFeatureIndexLayer.Projection = lInputGrid.Projection
    '        End If

    '        Dim lSum As Double = 0
    '        For lCol As Integer = lStartCol To lEndCol
    '            For lRow As Integer = lStartRow To lEndRow
    '                Dim lXYPos As DotSpatial.Topology.Coordinate = DotSpatial.Data.RasterExt.CellToProj(lInputGrid, lCol, lRow)
    '                Dim lSubId As Integer = aPolygonLayer.CoordinatesInShapefile(lXYPos.X, lXYPos.Y)
    '                GridMeanInPolygonFeatureIndexLayer.Value(lRow, lCol) = lSubId
    '                'If lSubId = aPolygonFeatureIndex Then 'this is in the polygon we want
    '                '    If Not lInputGrid.Value(lRow, lCol) < 0 Then
    '                '        lSum = lSum + lInputGrid.Value(lRow, lCol)
    ...
    '        GridMeanInPolygonFeatureIndexLayer.Save()
    '        GridMeanInPolygonFeatureIndexLayer.Close()
    ...
    ' ''' </summary>
    ' ''' <param name="aGridLayerIndex"></param>
    ' ''' <param name="aPolygonLayerIndex"></param>
    ' ''' <param name="aPolygonFeatureIndex"></param>
    ...
    ' ''' <returns></returns>
    ' ''' <remarks></remarks>
    'Public Shared Function GridSlopeInPolygon(ByVal aGridLayerIndex As Integer,
    '                                          ByVal aPolygonLayerIndex As Integer,
    '                                          ByVal aPolygonFeatureIndex As Integer,
    '                                          ByVal aRegionName As String) As Double
    '    Dim lInputGridName As String = LayerFileName(aGridLayerIndex)

    '    'Dim lRegion As New D4EM.Data.Region(lShape.ToShape, lPolygonSf.AsFeatureSet.Projection)
    '    'Dim lSlopeGridFileName As String = atcUtility.GetTemporaryFileName(IO.Path.Combine(PathNameOnly(lInputGridName), "slopeComputed" & aPolygonFeatureIndex), ".tif")
    '    Dim lSlopeGridFileName As String = IO.Path.Combine(PathNameOnly(lInputGridName), "slope_" & SafeFilename(aRegionName.Replace(" ", ""), "") & ".tif")
    '    Dim lSlopeLayer = D4EM.Geo.SpatialOperations.GetSlopeGrid(pMapWin.Layers(aGridLayerIndex), lSlopeGridFileName) ', lRegion)
    '    Return GridMeanInPolygon(lSlopeLayer, aPolygonLayerIndex, aPolygonFeatureIndex)

    'End Function

    ''' <summary>
    ''' Given a grid and a polygon layer, find the average grid value within the feature.
    ''' </summary>
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GridMeanInPolygon(ByVal aGrid As D4EM.Data.Layer,
                                             ByVal aPolygonLayerIndex As Integer,
                                             ByVal aPolygonFeatureIndex As Integer) As Double
        Dim lSlope = Double.NaN

        'set input polygon layer
        Dim lPolygonSf As D4EM.Data.Layer = PolygonShapeFileFromIndex(aPolygonLayerIndex)

        If FeatureIndexValid(aPolygonFeatureIndex, lPolygonSf) Then
            Dim lShape As DotSpatial.Data.Feature = lPolygonSf.AsFeatureSet.Features(aPolygonFeatureIndex)
            lSlope = D4EM.Geo.SpatialOperations.ComputeGridMeanInPolygon(aGrid, lShape, lPolygonSf.AsFeatureSet.ShapeIndices(aPolygonFeatureIndex), aPolygonFeatureIndex, 0, 200)
            lSlope /= 100   'convert from percent
        End If
        lPolygonSf = Nothing
        Return lSlope
    End Function
    ...
    '    'set input grid
    '    Dim gridLayer As DotSpatial.Data.Raster = LayerFromIndex(aGridLayerIndex).AsRaster
    '    Dim lRCindex As DotSpatial.Data.RcIndex = DotSpatial.Data.RasterExt.ProjToCell(gridLayer, aX, aY)

    '    If gridLayer.Value(lRCindex.Row, lRCindex.Column) < -9999999999 Then
    '        Return -999
    '    Else
    '        Return gridLayer.Value(lRCindex.Row, lRCindex.Column)
    '    End If
    'End Function
    ...
    ''' <param name="aLayer1Name">Name of first layer to overlay</param>
    ''' <param name="aLayer1FieldName">Name of field in first layer to overlay</param>
    ''' <param name="aLayer2Name">Name of second layer to overlay</param>
    ''' <param name="aLayer2FieldName">Name of field in second layer to overlay</param>
    ''' <param name="aOutputLayerName">Name of output layer</param>
    ''' <param name="aCreateNew">Flag, true if a new output shape file is desired, false if append existing file is desired</param>
    ...
    Public Shared Sub Overlay(ByVal aLayer1Index As Integer, ByVal aLayer1FieldIndex As Integer, _
        ...
        Dim lLayer1 As D4EM.Data.Layer = LayerFromIndex(aLayer1Index)
        Dim lSf1 As DotSpatial.Data.FeatureSet = lLayer1.AsFeatureSet

        'set layer 2 (subbasins)
        Dim lLayer2 As D4EM.Data.Layer = LayerFromIndex(aLayer2Index)
        Dim lSf2 As DotSpatial.Data.FeatureSet = lLayer2.AsFeatureSet
        Dim lSf2Ext As DotSpatial.Data.Extent = lSf2.Extent
        ...
        Dim lSfOut As New DotSpatial.Data.FeatureSet(lSf1.FeatureType)
        If aCreateNew Then 'create new output overlay shapefile
            'fixed to be able to use string 
            Dim lNewField1 As New System.Data.DataColumn
            With lNewField1
                .ColumnName = FieldName(aLayer1FieldIndex, aLayer1Index)
                .MaxLength = 10
            End With
            lSfOut.DataTable.Columns.Add(lNewField1)
            Dim lNewField2 As New System.Data.DataColumn
            With lNewField2
                .ColumnName = FieldName(aLayer2FieldIndex, aLayer2Index)
                .MaxLength = 10
            End With
            lSfOut.DataTable.Columns.Add(lNewField2)
            Dim lNewField3 As New System.Data.DataColumn
            With lNewField3
                .ColumnName = "Area"
                .MaxLength = 30
            End With
            lSfOut.DataTable.Columns.Add(lNewField3)
            lSfOut.SaveAs(aOutputLayerName, True)
        ...
        Dim lTotalPolygonCount As Integer = lSf1.Features.Count * lSf2.Features.Count
        Dim lPolygonCount As Integer = 0
        Dim lNumShapes As Integer = lSf1.Features.Count
        ...
        For lShapeIndex1 As Integer = 0 To lNumShapes - 1 'loop through each shape of the land use layer
                ...
                For lShapeIndex2 As Integer = 0 To lSf2.Features.Count - 1
                    ...
                    If Not (lSf1Ext.MinX > lSf2ShapeExtXmax(lShapeIndex2) OrElse
                        ...
                        If Not lShapeNew Is Nothing AndAlso lShapeNew.NumPoints > 0 Then 'Insert the shape into the shapefile 
                            ...
                            Dim lFeature1Id As String = FieldValue(aLayer1Index, lShapeIndex1, aLayer1FieldIndex)
                            Dim lFeature2Id As String = FieldValue(aLayer2Index, lShapeIndex2, aLayer2FieldIndex)
                            lSfOut.AddFeature(lShapeNew)
                            Dim lNewIndex As Integer = lSfOut.Features.Count - 1
                            lSfOut.Features(lNewIndex).DataRow(0) = lFeature1Id
                            lSfOut.Features(lNewIndex).DataRow(1) = lFeature2Id
                            lSfOut.Features(lNewIndex).DataRow(2) = lArea
                        End If
                        lShapeNew = Nothing
                    End If
            ...
            Else 'quick processing of all polygons in layer2 because outside of extent
                lPolygonCount += lSf2.Features.Count
            End If
    ...
    '    Dim lShapeNew As DotSpatial.Data.Feature
    '    Dim lShape1 As DotSpatial.Data.Feature
    '    Dim lShape1Ext As MapWinGIS.Extents
    '    Dim lShape2 As DotSpatial.Data.Feature
    '    Dim lShape2Ext As MapWinGIS.Extents

    '    lShape2 = lSf2.AsFeatureSet.Features(aLayer2SelectedIndex)
    '    lShape1 = lSf1.AsFeatureSet.Features(aLayer1SelectedIndex)
    ...
    '        If Not lShapeNew Is Nothing AndAlso lShapeNew.NumPoints > 0 Then 'Insert the shape into the shapefile 
    '            lBsuc = lSfOut.EditInsertShape(lShapeNew, lSfOut.AsFeatureSet.Features.Count)
    '            If Not lBsuc Then
    ...
    '        lShape2 = lSf2.AsFeatureSet.Features(aSelectedLayer2Indexes(k))
    '        lShape2Ext = lShape2.Extents
    ...
    '    Dim lTotalPolygonCount As Integer = lSf1.AsFeatureSet.Features.Count * aSelectedLayer2Indexes.Count
    '    Dim lPolygonCount As Integer = 0
    '    Dim lNumShapes As Integer = lSf1.AsFeatureSet.Features.Count
    '    For i As Integer = 1 To lNumShapes 'loop through each shape of the land use layer
    '        lShape1 = lSf1.AsFeatureSet.Features(i - 1)
    ...
    '    Dim lSf As D4EM.Data.Layer = ShapeFileFromIndex(aInputLayerIndex)

    '    'set clipper layer (subbasins for instance)
    '    Dim lSfClip As D4EM.Data.Layer = ShapeFileFromIndex(aClipperLayerIndex)
    '    Dim lShapeClip As DotSpatial.Data.Feature
    ...
    '        lNewShapeFileName = lPathClip & "\stream" & i & ".shp"
    '    Loop
    '    Dim lNewShapeFile As New D4EM.Data.Layer
    '    Dim rsf As New D4EM.Data.Layer
    '    lNewShapeFile.CreateNew(lNewShapeFileName, lSf.AsFeatureSet.FeaturesfileType)
    '    Dim lInputProjectionFileName As String = FilenameSetExt(lSf.FileName, "prj")
    '    If FileExists(lInputProjectionFileName) Then
    ...
    '    Dim issf As New D4EM.Data.Layer
    '    If Not issf.CreateNew("temp_clip.shp", lSf.AsFeatureSet.FeaturesfileType) Then
    '        Logger.Dbg("Failed to create temporary shapefile " & issf.FileName & " ErrorCode " & issf.LastErrorCode)
    '    End If

    '    lCount = 0
    '    lTotal = lSfClip.AsFeatureSet.Features.Count * lSf.AsFeatureSet.Features.Count
    '    lSfClip.BeginPointInShapefile()
    '    For i = 1 To lSfClip.AsFeatureSet.Features.Count
    '        lShapeClip = lSfClip.AsFeatureSet.Features(i - 1)
    '        For j = 1 To lSf.AsFeatureSet.Features.Count
    '            lCount += 1
    '            If pStatusShow Then Logger.Progress("Clipping Shapes with Polygon...", lCount, lTotal)
    '            If IsLineInPolygon(lSf.AsFeatureSet.Features(j - 1), lSfClip, i - 1) Then
    '                'at least one point of the line is in the polygon
    '                If IsLineEntirelyInPolygon(lSf.AsFeatureSet.Features(j - 1), lSfClip, i - 1) Then
    '                    'just add to output shapefile
    '                    lRetc = lNewShapeFile.EditInsertShape(lSf.AsFeatureSet.Features(j - 1), lNewShapeFile.AsFeatureSet.Features.Count)
    '                    'populate attributes of output shape
    '                    For k = 1 To lSf.AsFeatureSet.DataTable.Columns.Count
    '                        lRetc = lNewShapeFile.EditCellValue(k - 1, lNewShapeFile.AsFeatureSet.Features.Count - 1, lSf.CellValue(k - 1, j - 1))
    '                    Next k
    '                Else
    '                    'need to clip
    '                    Dim lTempShapeIndex As Integer = j - 1
    '                    lRetc = issf.EditInsertShape(lSf.AsFeatureSet.Features(j - 1), lTempShapeIndex)
    '                    lRetc = MapWinGeoProc.SpatialOperations.ClipShapesWithPolygon(issf, lShapeClip, rsf, True)
    '                    lRetc = issf.EditDeleteShape(0)
    '                    If rsf.AsFeatureSet.Features.Count > 0 Then
    '                        lRetc = lNewShapeFile.EditInsertShape(rsf.AsFeatureSet.Features(0), lNewShapeFile.AsFeatureSet.Features.Count)
    '                        'populate attributes of output shape
    '                        For k = 1 To lSf.AsFeatureSet.DataTable.Columns.Count
    '                            lRetc = lNewShapeFile.EditCellValue(k - 1, lNewShapeFile.AsFeatureSet.Features.Count - 1, lSf.CellValue(k - 1, j - 1))
    '                        Next k
    ...
    'Private Shared Function IsLineInPolygon(ByVal aLineShape As DotSpatial.Data.Feature, _
    '                                        ByVal aPolyShapeFile As D4EM.Data.Layer, _
    '                                        ByVal aIndex As Integer) As Boolean
    '    For i As Integer = 0 To aLineShape.numPoints - 1
    '        If aPolyShapeFile.PointInShapefile(aLineShape.Coordinates(i).X, _
    '                                           aLineShape.Coordinates(i).Y) = aIndex Then
    ...
    'Private Shared Function IsLineEntirelyInPolygon(ByVal aLineShape As DotSpatial.Data.Feature, _
    '                                                ByVal aPolyShapeFile As D4EM.Data.Layer, _
    '                                                ByVal aIndex As Integer) As Boolean
    '    For i As Integer = 0 To aLineShape.numPoints - 1
    '        If Not aPolyShapeFile.PointInShapefile(aLineShape.Coordinates(i).X, _
    '                                               aLineShape.Coordinates(i).Y) = aIndex Then
    ...
    Private Shared Function IsThisShapeTheSameAsOrPartOfAnotherShape(ByVal lSf1Shape As DotSpatial.Data.Feature, _
                                                                     ByVal lSf2Shape As DotSpatial.Data.Feature) As Boolean
    ...
    '    Dim i As Integer = 0
    '    Do While i < lsf.AsFeatureSet.Features.Count
    '        If pStatusShow Then Logger.Progress("Merging Features...", i, lsf.AsFeatureSet.Features.Count)
    '        lFound = False
    '        Dim lShape1 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(i)
    '        Dim lTargetVal As Integer = FieldValue(aLayerIndex, i, aFieldIndex)
    '        Dim lEndX As Double = lShape1.Coordinates(lShape1.NumPoints - 1).X
    '        Dim lEndY As Double = lShape1.Coordinates(lShape1.NumPoints - 1).Y
    '        Dim j As Integer = 0
    '        Do While j < lsf.AsFeatureSet.Features.Count
    '            If i <> j Then
    '                Dim lShape2 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(j)
    '                Dim lVal As Integer = FieldValue(aLayerIndex, j, aFieldIndex)
    '                If lVal = lTargetVal Then
    '                    'see if these have common start/end 
    '                    If lEndX = lShape2.Coordinates(0).X And lEndY = lShape2.Coordinates(0).Y Then
    '                        'end of shape 1 is start of shape2
    '                        For k As Integer = 1 To lShape2.NumPoints
    '                            lRetc = lShape1.InsertPoint(lShape2.Coordinates(k), lShape1.NumPoints)
    '                        Next k
    '                        'remove shape2
    ...
    '            Dim lShape1 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(i)
    '            Dim lTargetVal As Integer = FieldValue(aLayerIndex, i, aFieldIndex)
    '            lStartX1 = lShape1.Coordinates(0).X
    '            lStartY1 = lShape1.Coordinates(0).Y
    '            lEndX1 = lShape1.Coordinates(lShape1.NumPoints - 1).X
    '            lEndY1 = lShape1.Coordinates(lShape1.NumPoints - 1).Y
    '            Dim j As Integer = 0
    '            Do While j < lsf.AsFeatureSet.Features.Count
    '                If i <> j Then
    '                    Dim lShape2 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(j)
    '                    Dim lVal As Integer = FieldValue(aLayerIndex, j, aFieldIndex)
    '                    If lVal = lTargetVal Then
    '                        lStartX2 = lShape2.Coordinates(0).X
    '                        lStartY2 = lShape2.Coordinates(0).Y
    '                        lEndX2 = lShape2.Coordinates(lShape2.NumPoints - 1).X
    '                        lEndY2 = lShape2.Coordinates(lShape2.NumPoints - 1).Y
    ...
    '                                lRetc = lShape2.InsertPoint(lShape1.Coordinates(k), lShape2.NumPoints)
    '                            Next k
    '                            'remove shape1
    ...
    '    If pStatusShow Then Logger.Progress(lsf.AsFeatureSet.Features.Count, lsf.AsFeatureSet.Features.Count)
    'End Sub
    ...
    '        Dim lShape1 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(lSelectedShapeIndexes(0))
    '        Dim lShape2 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(lSelectedShapeIndexes(1))
    '        Dim lResultShape As New DotSpatial.Data.Feature
    ...
    '        lSuccess = lsf.EditInsertShape(lResultShape, lsf.AsFeatureSet.Features.Count)
    '        'set the attributes of the new shape to those of the first shape
    '        For lFieldIndex As Integer = 1 To lsf.AsFeatureSet.DataTable.Columns.Count
    '            lSuccess = lsf.EditCellValue(lFieldIndex - 1, lsf.AsFeatureSet.Features.Count - 1, lsf.CellValue(lFieldIndex - 1, lSelectedShapeIndexes(1)))
    '        Next
    '    End If
    ...
    '            Dim lShape1 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(lsf.AsFeatureSet.Features.Count - 1)
    '            Dim lShape2 As DotSpatial.Data.Feature = lsf.AsFeatureSet.Features(lSelectedShapeIndexes(lIndex))
    '            'merge these 2 shapes
    '            Dim lResultShape As New DotSpatial.Data.Feature
    '            lResultShape.Create(lsf.ShapefileType)
    '            Dim lSuccess As Boolean = MapWinGeoProc.SpatialOperations.MergeShapes(lShape1, lShape2, lResultShape)
    '            'delete the one at the end
    '            lSuccess = lsf.EditDeleteShape(lsf.AsFeatureSet.Features.Count - 1)
    '            'add this shape to the end
    '            lSuccess = lsf.EditInsertShape(lResultShape, lsf.AsFeatureSet.Features.Count)
    '            'set the attributes of the new shape to those of the first shape
    '            For lFieldIndex As Integer = 1 To lsf.AsFeatureSet.DataTable.Columns.Count
    '                lSuccess = lsf.EditCellValue(lFieldIndex - 1, lsf.AsFeatureSet.Features.Count - 1, lsf.CellValue(lFieldIndex - 1, lSelectedShapeIndexes(lIndex)))
    '            Next
    '        Next
    '    End If
    ...
    '        Dim mergeShp As DotSpatial.Data.Feature

    '        Dim sfSource As New D4EM.Data.Layer
    ...
    '            If Not .EditInsertShape(.AsFeatureSet.Features(lSelectedShapeIndexes(0)), .AsFeatureSet.Features.Count) Then Return False

    '            For i As Integer = 1 To lSelectedShapeIndexes.Count - 1
    '                Dim newShp As New DotSpatial.Data.Feature
    '                If Not newShp.Create(.ShapefileType) Then Return False
    '                If Not MapWinGeoProc.SpatialOperations.MergeShapes(sfSource, lSelectedShapeIndexes(i), .AsFeatureSet.Features.Count - 1, newShp) Then Return False
    '                'replace last shape with this one
    '                If Not .EditDeleteShape(.AsFeatureSet.Features.Count - 1) Then Return False
    '                If Not .EditInsertShape(newShp, .AsFeatureSet.Features.Count) Then Return False
    ...
    '            mergeShp = .AsFeatureSet.Features(.AsFeatureSet.Features.Count - 1)
    ...
    '        Dim sfDest As New D4EM.Data.Layer
    '        With sfDest
    '            If Not .Open(LayerFileName(aLayerIndexDest)) Then Return False
    '            If Not .StartEditingShapes(True) Then Return False
    '            If Not .EditInsertShape(mergeShp, .AsFeatureSet.Features.Count) Then Return False
    ...
    ' ''' <remarks>Programmed by LCW for GBMM project</remarks>
    '<CLSCompliant(False)> _
    'Public Shared Function FilterShape(ByRef shp As DotSpatial.Data.Feature, ByVal FilterSize As Single) As Boolean
    '    Try
    '        With shp
    '            Dim lstPoints As New Generic.List(Of MapWinGIS.Point)
    '            lstPoints.Add(.Coordinates(0))
    '            Dim lastPt As MapWinGIS.Point = .Coordinates(0)
    '            For i As Integer = 1 To .numPoints - 2
    '                With .Coordinates(i)
    '                    Dim dist As Single = Math.Sqrt((.x - lastPt.x) ^ 2 + (.y - lastPt.y) ^ 2)
    '                    If dist > FilterSize Then
    '                        lstPoints.Add(shp.Coordinates(i))
    '                        lastPt = shp.Coordinates(i)
    '                    End If
    '                End With
    '            Next
    '            lstPoints.Add(.Coordinates(.numPoints - 1))
    '            Dim shpNew As New DotSpatial.Data.Feature
    ...
    '    Dim lSf1 As D4EM.Data.Layer
    '    lSf1 = lLayer.GetObject
    '    Dim lShape1 As DotSpatial.Data.Feature
    '    lShape1 = lSf1.AsFeatureSet.Features(aLayer1FeatureIndex)

    '    'set layer 2 
    '    Dim lLayer2 As MapWindow.Interfaces.Layer
    '    lLayer2 = LayerFromIndex(aLayer2index)
    '    Dim lsf2 As D4EM.Data.Layer
    '    lsf2 = lLayer2.GetObject
    '    Dim lShape2 As DotSpatial.Data.Feature
    '    lShape2 = lsf2.AsFeatureSet.Features(aLayer2FeatureIndex)

    '    Dim lNewShape As DotSpatial.Data.Feature
    ...
    '    Dim sf As New D4EM.Data.Layer
    '    Dim tollSF As New D4EM.Data.Layer
    '    Dim fld As New MapWinGIS.Field
    '    Dim seg As New DotSpatial.Data.Feature
    '    Dim i As Integer
    ...
    '        'add all fields
    '        Dim j As Integer
    '        For j = 0 To sf.AsFeatureSet.DataTable.Columns.Count - 1
    '            tollSF.EditInsertField(sf.Field(j), j)
    '        Next

    '        For i = 1 To aSelectedAreaIndexes.Count

    '            seg = sf.AsFeatureSet.Features(aSelectedAreaIndexes(i))

    '            'Insert it into the new shapefile
    '            lStatus = tollSF.EditInsertShape(seg, i - 1)

    '            Dim h As Integer
    '            For h = 0 To sf.AsFeatureSet.DataTable.Columns.Count - 1
    ...
    '    Dim lVal As Object
    '    For i = 0 To lSf.AsFeatureSet.Features.Count - 1
    ...
    '        Dim lBrk As New D4EM.Data.LayerColorBreak
    ...
    '    Dim lShape As New DotSpatial.Data.Feature
    '    Dim lExtentsSf As New D4EM.Data.Layer
    ...
    '                                       ByVal aOutputProjection As String)
    '    Dim lShapefile As New D4EM.Data.Layer
    ...
    '        lField.Width = 10
    '        If Not lShapefile.EditInsertField(lField, lShapefile.AsFeatureSet.DataTable.Columns.Count) Then
    ...
    '            End If
    '            If Not lShapefile.EditInsertShape(lShape, lShapefile.AsFeatureSet.Features.Count) Then
    '                Logger.Dbg("Failed to insert point shape into shapefile.")
    '            End If

    '            For lAttributeIndex = 1 To lAttributeCount
    '                If Not lShapefile.EditCellValue(lAttributeIndex - 1, lShapefile.AsFeatureSet.Features.Count - 1, aAttributeValues((lIndex * lAttributeCount) + lAttributeIndex)) Then
    ...
    '    lPt = MapWinGeoProc.Utils.Centroid(lSf.AsFeatureSet.Features(aFeatureIndex))
    '    aCentroidX = lPt.x
    '    aCentroidY = lPt.y
    'End Sub
    ...
    '    Dim lSf As New D4EM.Data.Layer
    '    lSf = ShapeFileFromIndex(aLayerIndex)
    '    Dim lShape As DotSpatial.Data.Feature = lSf.AsFeatureSet.Features(aFeatureIndex)
    '    aCentroidX = ((lShape.AsFeatureSet.Extent.MaxX - lShape.Envelope.X) / 2) + lShape.Envelope.X
    '    aCentroidY = ((lShape.AsFeatureSet.Extent.MaxY - lShape.Envelope.Y) / 2) + lShape.Envelope.Y
    'End Sub
    ...
    '    For lIndex = 0 To lSf.AsFeatureSet.Features.Count - 1
    ...
    '        Dim lBreak As New D4EM.Data.LayerColorBreak
    ...
    '    Dim lInSf As New D4EM.Data.Layer
    '    lInSf.Open(aInputShapefileFilename)
    '    Dim lOutSf As New D4EM.Data.Layer
    '    If lOutSf.CreateNew(aResultShapefileFilename, MapWinGIS.ShpfileType.SHP_POLYGON) Then
    '        For lFieldIndex As Integer = 1 To lInSf.AsFeatureSet.DataTable.Columns.Count
    '            lOutSf.EditInsertField(lInSf.Field(lFieldIndex - 1), lFieldIndex - 1)
    '        Next lFieldIndex
    '        lOutSf.StartEditingShapes()
    '        For lIndex As Integer = 0 To lInSf.AsFeatureSet.Features.Count - 1
    '            Dim lShape As DotSpatial.Data.Feature = lInSf.AsFeatureSet.Features(lIndex)
    '            Dim lNewShape As New DotSpatial.Data.Feature
    '            lNewShape = lShape.Buffer(aBufferDistance, 0)
    '            lOutSf.EditInsertShape(lNewShape, lOutSf.AsFeatureSet.Features.Count)
    '            For lFieldIndex As Integer = 0 To lInSf.AsFeatureSet.DataTable.Columns.Count - 1
    '                lOutSf.EditCellValue(lFieldIndex, lOutSf.AsFeatureSet.Features.Count - 1, lInSf.CellValue(lFieldIndex, lIndex))
    '            Next lFieldIndex
    '        Next
    '    End If
    ...
    '        'yes
    '        Dim lPolygonShapefile As D4EM.Data.Layer = ShapeFileFromIndex(aPolygonLayerIndex)
    '        Dim lOutputPoint As New MapWinGIS.Point
    '        If FeatureIndexValid(lPolygonFeatureIndex, lPolygonShapefile) Then
    '            Dim lContainingShape As DotSpatial.Data.Feature = lPolygonShapefile.AsFeatureSet.Features(lPolygonFeatureIndex)
    '            Dim lLocation As Integer
    '            Dim lDistance As Double
    ...
    '    Dim lSourceSf As D4EM.Data.Layer = ShapeFileFromIndex(aSourceLayerIndex)
    '    Dim lTargetSf As D4EM.Data.Layer = ShapeFileFromIndex(aTargetLayerIndex)
    ...
    '    Else
    '        For lSourceFieldIndex As Integer = 0 To lSourceSf.AsFeatureSet.DataTable.Columns.Count - 1
    ...
    Public Shared ReadOnly Property ShapefileProjectionString(Optional ByVal aLayerIndex As Integer = UseCurrent) As String
        Get
            Return ShapeFileFromIndex(aLayerIndex).AsFeatureSet.Projection.ToProj4String
        End Get
    End Property
    ...
    '    Get
    '        Return GetMappingObject.View.AsFeatureSet.Extent.MaxX
    '    End Get
    'End Property

    'Public Shared ReadOnly Property MapExtentXmin() As Double
    '    Get
    '        Return GetMappingObject.View.Envelope.X
    '    End Get
    'End Property

    'Public Shared ReadOnly Property MapExtentYmax() As Double
    '    Get
    '        Return GetMappingObject.View.AsFeatureSet.Extent.MaxY
    '    End Get
    'End Property

    'Public Shared ReadOnly Property MapExtentYmin() As Double
    '    Get
    '        Return GetMappingObject.View.Envelope.Y
    '    End Get
    'End Property
...
#Region "Added by Chris Wilson during Sediment/Mercury WCS project"
    ...
    Public Shared ReadOnly Property MapUnits() As String
        Get
            'Return GetMappingObject.Project.MapUnits
            Return GetMappingObject.DesiredProjection.Unit.Name.ToString
        End Get
    End Property
    ...
    ''' <remarks>Added by Chris Wilson 11/25/2008</remarks>
    <CLSCompliant(False)> _
    Public Shared ReadOnly Property LayerType(ByVal aLayerName As String) As Integer
        Get
            Select Case IO.Path.GetExtension(LayerFromIndex(LayerIndex(aLayerName)).FileName).ToLower
                'Image As MapWindow.Interfaces.eLayerType = 0
                'PointShapefile As MapWindow.Interfaces.eLayerType = 1
                'LineShapefile As MapWindow.Interfaces.eLayerType = 2
                'PolygonShapefile As MapWindow.Interfaces.eLayerType = 3
                'Grid As MapWindow.Interfaces.eLayerType = 4
                Case ".shp" : Return 3
                Case ".tif" : Return 4
                Case Else
                    Return -1 'Invalid As MapWindow.Interfaces.eLayerType = -1
            End Select
        End Get
    End Property
    ...
    '    'set up collections of subbasin shapes and extents to save computation time later
    '    Dim lSf2Shape As New Generic.List(Of DotSpatial.Data.Feature)
    '    Dim lSf2ShapeExtXmax As New Generic.List(Of Double)
    '    Dim lSf2ShapeExtXmin As New Generic.List(Of Double)
    '    Dim lSf2ShapeExtYmax As New Generic.List(Of Double)
    '    Dim lSf2ShapeExtYmin As New Generic.List(Of Double)
    ...
    '        'loop thru each selected subbasin (or all if none selected)
    '        lShape2 = lSf2.AsFeatureSet.Features(k)
    '        lShape2Ext = lShape2.Extents
    '        lSf2Shape.Add(lShape2)
    '        lSf2ShapeExtXmax.Add(lShape2Ext.xMax)
    '        lSf2ShapeExtXmin.Add(lShape2Ext.xMin)
    ...
    '        lMinX = Math.Min(lMinX, lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.X)
    '        lMaxX = Math.Max(lMaxX, lPolygonSf.AsFeatureSet.Features(lPolyIndex).AsFeatureSet.Extent.MaxX)
    '        lMinY = Math.Min(lMinY, lPolygonSf.AsFeatureSet.Features(lPolyIndex).Envelope.Y)
    '        lMaxY = Math.Max(lMaxY, lPolygonSf.AsFeatureSet.Features(lPolyIndex).AsFeatureSet.Extent.MaxY)
    '    Next
    ...
    '                If lInputGrid.Value(lRow, lCol).GetType.Name = "SByte" Then
    '                    lGridValue = Convert.ToInt32(lInputGrid.Value(lRow, lCol))
    '                Else
    '                    lGridValue = lInputGrid.Value(lRow, lCol)
    '                End If
    ...
    '    Cancel = False
    '    Dim sfSubbasin As D4EM.Data.Layer = Nothing, sfLanduse As D4EM.Data.Layer = Nothing
    '    Try
    ...
    '        sfSubbasin = New D4EM.Data.Layer
    ...
    '        For i As Integer = 0 To sfSubbasin.AsFeatureSet.Features.Count - 1
    '            Dim NumErrors As Integer = 0
    '            Dim shpSubbasin As DotSpatial.Data.Feature = sfSubbasin.AsFeatureSet.Features(i)
    ...
    '            Dim jmax As Integer = sfLanduse.AsFeatureSet.Features.Count - 1
    ...
    '                        Dim shpLanduse As DotSpatial.Data.Feature = sfLanduse.AsFeatureSet.Features(j)
    '                        Try
    '                            Dim shpInt As DotSpatial.Data.Feature = MapWinGeoProc.SpatialOperations.Intersection(shpSubbasin, shpLanduse)
    ...
    '    Cancel = False
    '    Dim sfSubbasin As New D4EM.Data.Layer, gLandUse As New MapWinGIS.Grid
